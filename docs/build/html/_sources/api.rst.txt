...
get_candle_borders(), а получить исторические значения свечек с помощью get_candles(), используя числовой код размера
свечки.

.. autofunction:: aiomoex.get_candle_borders

.. autofunction:: aiomoex.get_candles

Исторические данные по дневным котировкам
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
В отличие от свечек, функции данного раздела предоставляют много вспомогательной информации кроме значений HLOCV.
