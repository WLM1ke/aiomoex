...
class ISSClientSession:
    ...
    Для ускорения работы рекомендуется иметь одну клиентскую сессию в течении всей работы приложения. В конце работы для
    высвобождения ресурсов сессия должна быть закрыта вызовом метода closed. В большинстве случаев удобнее
    воспользоваться синтаксисом асинхронного контекстного менеджера
    """
...
def _make_query(*, start=None, end=None, table=None, columns=None):
    ...
    :return: Словарь с дополнительными параметрами запроса
    """
    query = dict()
    if start:
        query['from'] = start
...
async def get_market_history(security, start=None, end=None, columns=('TRADEDATE', 'CLOSE', 'VOLUME'),
    ...
    :param start: дата вида ГГГГ-ММ-ДД. При отсутствии данные будут загружены с начала истории
    :param end: дата вида ГГГГ-ММ-ДД. При отсутствии данные будут загружены до конца истории
